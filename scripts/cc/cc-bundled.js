...
// scripts/cc/lib/client.ts
var import_node_fs = require("node:fs");
var DEFAULT_CC_API_KEY_SECRET_FILE = "/run/secrets/cc-api-key";
...
var CommandCenterClient = class {
  static {
    __name(this, "CommandCenterClient");
  }
  apiKey;
  baseUrl;
  timeout;
...
function resolveApiKey(pathOverride) {
  const fromEnv = process.env.COMMAND_CENTER_API_KEY;
  if (fromEnv && fromEnv.trim() !== "") return fromEnv.trim();
  const secretPath = pathOverride ?? process.env.CC_API_KEY_SECRET_FILE ?? DEFAULT_CC_API_KEY_SECRET_FILE;
  try {
    const contents = (0, import_node_fs.readFileSync)(secretPath, "utf8").trim();
...
function createClient(urlOverride) {
  ...
  if (!apiKey) {
    ...
    console.error(
      `     (In Managed Agents sessions, mount the key as a file resource at ${DEFAULT_CC_API_KEY_SECRET_FILE}.)`
    );
    process.exit(1);
  }
...
// scripts/cc/lib/envelope.ts
var isAgent = !process.stdout.isTTY;
function respond(command, result, nextActions = []) {
  if (isAgent) {
...
function respondError(command, message, code, fix, nextActions = []) {
  if (isAgent) {
...
__name(respondError, "respondError");
...
// scripts/cc/commands/work-items.ts
var UUID_LENGTH = 36;
async function resolveId(client, shortId) {
  if (shortId.length >= UUID_LENGTH) return shortId;
  const data = await client.get("/api/work-items?limit=200");
...
function parseMetadataFlag(raw) {
...
function registerWorkItems(program3) {
  ...
  wi.command("list").description("List work items").option("-p, --project <project>", "Filter by project").option("-s, --status <status>", "Filter by status (comma-separated)", "discovered,triaged,approved,in_progress,review").option("-l, --limit <n>", "Max results", "50").option("--type <type>", "Filter by type (comma-separated)").option("--include-initiatives", "Include initiative/sprint container items (excluded by default)").action(async (opts) => {
    try {
      ...
      const data = await client.get(`/api/work-items?${params}`);
...
`);
  ...
  wi.command("create").description("Create a work item").requiredOption("-t, --title <title>", "Title").option("-d, --description <desc>", "Description").option("-p, --project <project>", "Project name").option("--priority <priority>", "Priority: high, medium, low", "medium").option("--type <type>", "Type: task, finding, initiative, research_request", "task").option("--source <source>", "Source type: human, agent", "human").option("--assigned-to <who>", "Assign to").option("--created-by <who>", "Created by", "clay").option(
    ...
    try {
      ...
      const data = await client.post("/api/work-items", body);
      respond("cc work-items create", data, [
        ...data.created.length > 0 ? [{ command: `cc wi list --project=${opts.project || "all"}`, description: "List items in project" }] : [],
        ...data.skipped.length > 0 ? [{ command: `cc wi list --status=discovered,triaged`, description: "View open items (duplicate found)" }] : []
      ]);
      if (!isAgent) {
  ...
  wi.command("bulk-create").description("Create multiple work items from JSON").argument("<json>", "JSON array of items or path to JSON file").action(async (json) => {
    try {
      const client = createClient(program3.opts().url);
      let items;
      try {
      ...
      const data = await client.post("/api/work-items", { items });
  ...
  wi.command("bulk-close").description("Close multiple work items").argument("<ids...>", "Work item IDs").option("-s, --status <status>", "Close status: done, rejected, deferred", "done").option("--notes <notes>", "Closure notes").option("--actor <actor>", "Actor name", "clay").action(async (ids, opts) => {
    try {
      const client = createClient(program3.opts().url);
      const fullIds = await resolveIds(client, ids);
      const data = await client.patch("/api/work-items", {
        ids: fullIds,
        status: opts.status,
        notes: opts.notes,
        actor: opts.actor
...
__name(registerWorkItems, "registerWorkItems");

// scripts/cc/commands/context-items.ts
function registerContextItems(program3) {
  const ctx = program3.command("context").alias("ctx").description("Ingest context items (meeting notes, emails)");
  ctx.command("ingest").description("Ingest text content \u2014 AI extracts summary, action items, and spawns work items").requiredOption("-c, --content <text>", "Raw text content").option("-s, --source <source>", "Source type: email, meeting, feedback", "meeting").option("-p, --project <project>", "Project name (auto-detected if omitted)").option("--account <account>", "Source account (email or meeting ID)").option("--people <people>", "Comma-separated names of people involved").action(async (opts) => {
    try {
      const client = createClient(program3.opts().url);
      const body = {
        source: opts.source,
      ...
      if (opts.people) body.people = opts.people.split(",").map((p) => p.trim());
      const data = await client.post("/api/context-items", body);
      respond("cc context ingest", data, [
        ...data.work_item_ids.length > 0 ? [{ command: `cc wi list --status=discovered`, description: "View spawned work items" }] : [],
  ...
  ctx.command("ingest-file").description("Ingest content from a file").argument("<file>", "Path to text file").option("-s, --source <source>", "Source type: email, meeting, feedback", "meeting").option("-p, --project <project>", "Project name").option("--people <people>", "Comma-separated names").action(async (file, opts) => {
    try {
      const fs = await import("fs");
...
__name(registerContextItems, "registerContextItems");

// scripts/cc/commands/sync.ts
function registerSync(program3) {
  const sync = program3.command("sync").description("Sync integrations and data sources");
  sync.command("app-store").description("Sync App Store Connect metrics (downloads, revenue)").option("--date <date>", "Report date (YYYY-MM-DD)", (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)).action(async (opts) => {
    try {
      const client = createClient(program3.opts().url);
  ...
  sync.command("meta-ads").description("Sync Meta ad campaign metrics").option("-m, --month <month>", "Month (YYYY-MM)", (/* @__PURE__ */ new Date()).toISOString().slice(0, 7)).action(async (opts) => {
    try {
      const client = createClient(program3.opts().url);
      const data = await client.post(
  ...
  sync.command("glossy-feedback").description("Sync Glossy Sports user feedback into Command Center").action(async () => {
    try {
      const client = createClient(program3.opts().url);
      const data = await client.post(
  ...
  sync.command("glossy-feedback-status").description("Check Glossy Sports feedback sync status").action(async () => {
    try {
      const client = createClient(program3.opts().url);
      ...
      respond("cc sync glossy-feedback-status", data, [
        ...data.pending > 0 ? [{ command: `cc sync glossy-feedback`, description: `Sync ${data.pending} pending items` }] : []
      ]);
      if (!isAgent) {
  ...
  sync.command("loop-closure").description("Run loop closure sweep (consolidate agent findings)").action(async () => {
    try {
      const client = createClient(program3.opts().url);
      const data = await client.post(
...
__name(registerSync, "registerSync");
...
registerWorkItems(program2);
registerContextItems(program2);
