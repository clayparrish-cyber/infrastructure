...
# Look up autonomy level for a decision category
lookup_autonomy_level() {
  local decision_category="$1"
  [ -z "$decision_category" ] && echo "1" && return
  curl -s \
    -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
    -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
    "$SUPABASE_URL/rest/v1/autonomy_rules?decision_category=eq.$decision_category&select=current_level&limit=1" \
    | python3 -c "import sys,json; d=json.load(sys.stdin); print(d[0]['current_level'] if d else 1)" 2>/dev/null || echo "1"
}

# Create a PR for a worker branch and optionally enable auto-merge for L3+ items
create_pr_and_maybe_merge() {
  local item_id="$1" item_json="$2" branch_name="$3" execution_log="$4"
  local project title decision_category pr_url autonomy_level
  ...
  decision_category=$(echo "$item_json" | python3 -c "import sys,json; print(json.load(sys.stdin).get('decision_category',''))")

  # Verify branch exists on remote
  if ! git ls-remote --heads origin "$branch_name" 2>/dev/null | grep -q "$branch_name"; then
    log "PR SKIP: $item_id — branch $branch_name not found on remote"
    return 1
  fi
  ...
  # Create PR
  local pr_body
  pr_body="Automated fix for work item \`${item_id}\`

${execution_log}

---
Generated by nightly worker agent"
  ...
  pr_url=$(gh pr create \
    ...
    log "PR FAILED: $item_id — gh pr create error: $pr_url"
    return 1
  }

  log "PR CREATED: $item_id → $pr_url"

  # Store PR URL in work item metadata
  update_work_item "$item_id" "{\"metadata\":$(echo "$item_json" | python3 -c "
import sys, json
item = json.load(sys.stdin)
meta = item.get('metadata') or {}
meta['pr_url'] = '$pr_url'
meta['auto_merge_requested'] = False
print(json.dumps(meta))
"),\"updated_at\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\"}"

  # Check autonomy level for auto-merge
  autonomy_level=$(lookup_autonomy_level "$decision_category")

  if [ "$autonomy_level" -ge 3 ] 2>/dev/null; then
    log "AUTO-MERGE: $item_id — L$autonomy_level, requesting auto-merge"
...
meta = item.get('metadata') or {}
meta['pr_url'] = '$pr_url'
meta['auto_merge_requested'] = True
print(json.dumps(meta))
"),\"updated_at\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\"}"
      insert_event "$item_id" "auto_merge_requested" "review" "review" "Auto-merge enabled for L$autonomy_level PR: $pr_url"
    else
      log "AUTO-MERGE UNAVAILABLE: $item_id — repo may lack branch protection. PR stays open."
      insert_event "$item_id" "auto_merge_failed" "review" "review" "Auto-merge not available (no branch protection?). PR open: $pr_url"
    fi
  else
    log "MANUAL REVIEW: $item_id — L$autonomy_level, PR open for review"
  fi
